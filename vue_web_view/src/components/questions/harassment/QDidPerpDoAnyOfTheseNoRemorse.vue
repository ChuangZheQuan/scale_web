<template>
  <v-carousel-item
    ...
    :value="$store.state.FLOW['H_DID_PERP_DO_ANY_OF_THESE_NO_REMORSE']"
  >
...
@Component
export default class QDidPerpDoAnyOfTheseNoRemorse extends Vue {
  ...
  next(ans: string): void {
    if (ans === this.answers[0]) {
      this.$emit("next", {
        value: this.$store.state.FLOW["POSSIBLY_S3_LIKELY_S4"],
